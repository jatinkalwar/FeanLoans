...
async def getapplicationslist(token):
    try:
        ...
        documents = list(forms.find(
            ...
            {"status": 1, "fill_on": 1, "_id": 0 , 'application_no':1 , 'mobile':1 , 'name':1 , 'loan_amount':1}  # Projecting only 'status' and 'time', excluding '_id'
        ))
...
async def getagreementlist(token):
    try:
        ...
        documents = list(forms.find(
            ...
            {"fill_on": 1, "_id": 0 , 'application_no':1 , 'mobile':1 , 'name':1 , 'loan_amount':1 , 'agreement_amount':1} ))
